...
class CollegeModel extends Model {
	...
	{
		$sql = "SELECT college.c_name,college.c_id,college.img,college.xyjianjie,college.Principal,course.cname "
                        . "FROM college "
                        . "INNER JOIN major ON college.c_id = major.c_id "
                        . "INNER JOIN course on college.c_id = course.c_id "
                        . "group by c_name "
                        . "order by c_id asc "
                        . "limit 10";
	...
	Public function kec()
	{
		$kec=M("college")->query("SELECT cname,sid,cid,intro,intro,cimg,people,addtime,video,college.c_id,c_name FROM course INNER JOIN college on course.c_id=college.c_id ORDER BY people DESC LIMIT 12");
                return $kec;
	}
	//查询全部课程
	...
	public function get_course_level($id)
	{
		...
		return $arr;*/
		//分组查询，每个阶段查询代表课程
		$sql = "SELECT cname,sid,cimg,cid,b.* FROM course a left join teacher b on a.teacher_id=b.id WHERE a.c_id=$id  group by sid";
