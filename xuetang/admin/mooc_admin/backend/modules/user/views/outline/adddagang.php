...
<form class="dropzone dz-clickable" method="post" action="index.php?r=user/outline/save" enctype="multipart/form-data">
...
<div class="col-xs-2" style="margin-left:100px;">
                <select name="course" id="course"  class="form-control" onchange="ck_course()">
								<option value="0" selected>请选择
				</select>
                </div>
...
<div class="col-xs-2" style="margin-left:100px;">
                <select name="five" id="five"  class="form-control">
								<option value="0" selected>请选择
				</select>
                </div>
...
<font style="margin-left:-60%;color:#000;font-family:黑体; font-size:18px; font-weight:normal; margin-right:6px;" size="5">大纲标题:</font>
<div class="col-xs-2" style="margin-left:100px;">
    <textarea rows="2" cols="60" name="intro" ></textarea>
 </div>
...
<script>
	function ck_two(){ 
		...
		$.ajax({ 
			url:"index.php?r=user/outline/cktwo",
			data:{"one":one},
			dataType:"json",
			type:"post",
	...
	function ck_three(){ 
		var two=$("#two").val();
		$.ajax({ 
			url:"index.php?r=user/outline/ckthree",
			data:{"two":two},
			dataType:"json",
			type:"post",
			success:function(msg){ 
	...
	function ck_course(){
		...
		//$("#three").html('<option>'+three+'</option>');
		$.ajax({
			url:"index.php?r=user/outline/ckcourse",
			data:{"course":course},
			dataType:"json",
			type:"post",
			success:function(msg){
				...
				for(var i=0; i<msg.length;i++){ 
					str+="<option value="+msg[i].ch_id+" >"+msg[i].ch_name+"</option>";	
				}
		...
		$(function() {
			$('#video_upload').uploadify({
				...
				'uploader' :  'index.php?r=user/outline/save_doc',
                onUploadSuccess: function (event, response, status) {
                    if(response==1){
                        alert("请上传doc,docx,pdf,rtf格式大纲");
                    }
