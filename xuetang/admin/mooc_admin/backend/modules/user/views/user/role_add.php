...
<span>
<font style="margin-left:-60%;color:#000;font-family:黑体; font-size:18px; font-weight:normal; margin-right:6px;" size="5">选择权限:</font>
<div class="col-xs-2" style="margin-left:100px;">
	       <?php foreach ($arr as $one):?>
          <input type="checkbox" id="box" value="<?php echo $one['id']?>"><?php echo $one['name']?><br/>
        <?php endforeach;?>  
                </div>
...
<!--
<span>
<font style="margin-left:-59%;color:#000;font-family:黑体; font-size:18px; font-weight:normal; margin-right:650px;" size="5">确认密码:</font>
...
<script>
  $("#but").click(function(){
      var name=$("#user").val();
        ...
        $.ajax({
           type: "POST",
           url: "index.php?r=user/user/role_ad",
